...
# Fetch: L - 3318
# Geocode (f): L - 3352
# Universal_Caller(f): L - 3365
# Service: L - 3399
# Plug-in Manager: L - 3980
...
class GSearch(object):
    """
    Custom class for conducting geographic queries of sellers.
    
    Works with V3 of GMaps JS API
    
    Needs to take:
    * Search string
    * Co-ords of the buyer - geolocation using network routing or internal gps - Google Gears?
    
    Needs to return:
    * Formatted HTML to render to page.
    
    Too many limitations on the output of the strings, moving JS to the views. 
    ...
    #    self.i = "Ming mang"
    #    self.j = "Bing bang"
#Bound method -> called as unbound (http://stackoverflow.com/questions/114214/class-method-differences-in-python-bound-unbound-and-static)    
    #def testFunc(self):
    #    self.i = "Crazy Cunt"
    #    pass
    
    def navTest(self):
        self.i = "Bangalanglang"
...
class Auth(object):
    ...
    - James attempting to write a 'home' - herpa derpa
    ...
    def __init__(self, environment=None, db=None, mailer=True,
        ...
        settings.login_after_registration = True # JM
        settings.alternate_requires_registration = False
        settings.create_user_groups = True

        settings.controller = controller
        settings.login_url = URL('OJTest', 'default', 'index') # Edited by JM 12/27/2011
        settings.logged_url = URL('OJTest', 'default', 'index') # self.url('user', args='profile')
        ...
        # 10/5/2011 - edited by JM
        messages.register_button = 'Submit'
        ...
        messages.label_location_of_origin = 'Location of Origin'
        messages.label_date_joined = 'Date Joined'
        messages.label_user_image = 'User image'
        
        # Customised by JM - 10/5/2011
        #UsersDB custom messages
        messages.label_buyers_id = 'Buyer\'s ID'
        messages.label_sellers_id = 'Seller\'s ID'
        messages.label_remember_me = "Remember me (for 30 days)"
        messages['T'] = current.T
        # Customised by JM - 11/24/2011
        messages.label_usnum = 'Unit / Street Number'
        messages.label_street = 'Street'
        messages.label_suburb = 'Suburb'
        messages.label_state = 'State'
        messages.label_country = 'Country'
        messages.label_postcode = 'Postcode'
        # Customised by JM - 12/17/2011
        messages.label_reputation = 'Reputation'
        messages.label_region_id = 'Region ID'
        messages.label_user_type = 'User Type'
        #BuyersDB custom messages
        #SellersDB custom messages
        messages.verify_password_comment = 'Please enter your password again'
    ...
    def navbar(self, prefix='Tools -> Auth() -> navbar. This needs to be changed. Logged in as: ', action=None):
        request = current.request
        T = current.T
    ...
    def define_tables(self, username=False, migrate=True, fake_migrate=False):
        ...
        """
# Creates the Auth_user table
        db = self.db
        settings = self.settings 
        if not settings.table_user_name in db.tables:
            passfield = settings.password_field
#If user is identified by username:
...
# Customized fields by JM -   10/5/2011                                      
                    Field('sellers_id', length=512,
                          writable=False, readable=False, default='',
                          label=self.messages.label_sellers_id),
                    Field('date_joined', length=512,
                          writable=False, readable=False, default='',
                          label=self.messages.label_date_joined),
                    Field('location_of_origin', length=512,
                          writable=False, readable=False, default='',
                          label=self.messages.label_location_of_origin),                                        
# Customized fields by JM - 11/24/2011
                    Field('usnum', length=512,
                          writable=False, readable=False, default='',
                          label=self.messages.label_usnum),
                    Field('street', length=512,
                          writable=False, readable=False, default='',
                          label=self.messages.label_street),
                    Field('suburb', length=512,
                          writable=False, readable=False, default='',
                          label=self.messages.label_suburb),
                    Field('state', length=512,
                          writable=False, readable=False, default='',
                          label=self.messages.label_state),
                    Field('country', length=512,
                          writable=False, readable=False, default='',
                          label=self.messages.label_country),
                    Field('postcode', length=512,
                          writable=False, readable=False, default='',
                          label=self.messages.label_postcode),
# Customised fields by JM - 12/17/2011
                    Field('reputation',length=512,
                          writable=False,readable=False,default='',
                          label = self.messages.label_reputation),
                    Field('region_id',length=512,
                          writable=False,readable=False,default='',
                          label = self.messages.label_region_id),
# Customised fields by JM - 12/28/2011
                    Field('user_type',length=512,
                          writable=False,readable=False,default='',
                          label = self.messages.label_user_type),                                
...
#If username is not defined by username:
            else:
                table = db.define_table(
                    settings.table_user_name,
                    Field('first_name', length=128, default='',
...
# Creates Auth_Cas DB                
        ...
        if self.settings.login_form == self:
            ...
            if form.accepts(request, session,
                ...
                if not user:
                    ...
                    #redirect(URL('OJTest','default','index'))
    ...
    def logout(self, next=DEFAULT, onlogout=DEFAULT, log=DEFAULT):
        ...
        ## JM - 13/12/2011
        ## Need to write forget code for anything I set into a session
...
# Requires the user to register their email        
